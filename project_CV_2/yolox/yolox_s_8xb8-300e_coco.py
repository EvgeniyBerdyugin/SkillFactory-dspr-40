...
custom_hooks = [
    ...
    dict(
        ...
        type='EMAHook',
        update_buffers=True),
    dict(type='CollectMetricsHook_YOLOX'),
]
...
log_level = 'INFO'
log_processor = dict(by_epoch=True, type='LogProcessor', window_size=5)
max_epoch = 500
max_epochs = 300
model = dict(
    backbone=dict(
...
test_evaluator = dict(
    ann_file='data/masks_coco/val.json',
    backend_args=None,
    metric='bbox',
    type='CocoMetric')
test_pipeline = [
...
work_dir = 'work_dirs/yolox_custom/train_lr'
